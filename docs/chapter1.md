...
为什么是 `exec`、`read_file`、`write_file`、`edit_file` 这四个？

1. **最小完备集**：这四个工具构成了文件操作和命令执行的完整闭环
2. **组合性力量**：通过工具组合可以完成复杂任务，例如：
   ```
   查找文件 → 读取内容 → 搜索关键词 → 生成报告
   exec(find)    read_file     exec(grep)    调用LLM
   ```
...
**示例**：
- `exec("git status")` → 查看 Git 状态
- `exec("pip install requests")` → 安装 Python 包
- `exec("ls -la")` → 列出文件
...
```python
{"name": "exec", "description": "执行 shell 命令并返回输出。适用于系统命令、git、包管理器等。", ...}
```
- 清晰的描述帮助 LLM 准确选择工具
- 结构化参数定义确保调用格式正确
- 这是 **LLM 与工具系统** 的唯一接口
...
| **4 个工具** | exec / read_file / write_file / edit_file 的最小完备集 |
